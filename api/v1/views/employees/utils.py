...
def add_req_args(passwd=True):
    ...
    employee_args.add_argument("middle_name", type=str, help="middle name")
    employee_args.add_argument(
        "last_name", type=str, required=True, help="last name is required"
    )
    employee_args.add_argument("age", type=int, required=True,
                               help="age is required")
    employee_args.add_argument(
        "gender", type=str, required=True, help="gender is required"
    )
    employee_args.add_argument(
        "marital_status", type=str, required=True,
        help="marital status is required"
    )
    employee_args.add_argument(
    ...
    employee_args.add_argument(
        "password", type=str, required=passwd, help="password is required"
    )
    employee_args.add_argument(
        "street", type=str, required=True, help="street name is required"
    )
    employee_args.add_argument(
        "city", type=str, required=True, help="city name is required"
    )
    employee_args.add_argument(
        "state", type=str, required=True, help="state name is required"
    )
    employee_args.add_argument(
        "role", type=str, required=True, help="employee role is required"
    )
    employee_args.add_argument("salary", type=float)
    ...
    employee_args.add_argument("permissions", type=str, help="permissions")
    employee_args.add_argument(
        "warehouses",
        type=str,
        required=True,
...
def check_required_attributes(passwd=True):
    ...
    if (
        len(args["phone_number"]) != 11
        and len(args["guarantor_contact"]) != 11
        and len(args["warehouses"]) != 36
    ):
        abort(
            400,
